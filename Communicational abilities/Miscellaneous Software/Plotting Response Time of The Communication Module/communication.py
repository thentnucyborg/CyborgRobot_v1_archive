...
def plot_data(data):
	...
	print "__________________PLOTTING AS WE SPEAK________________"
	plt.plot(data)
	plt.ylabel("Computation time [secs]")
	plt.xlabel("Step number")

	plt.title("Response time of Cyborg's communication module")
...
def plot_average_computation_time(average_dict):
	#{"cleverbot_mode":[1.04984,101.29292],"v":[1.2]}
	...
	plt.bar(x_set_bar_titles,t,align="center",color="y")
	plt.xticks(x_set_bar_titles,x)
	plt.xlabel("Behavioral mode")
	plt.ylabel("Average computation time [secs]")
	plt.title("Average computation time for the several modes")

	axes = plt.gca()
	axes.set_ylim([0,max(t)+1])

	plt.show()

def return_average_computation_time(array_data):
	l = len(array_data)
	attr = []
	...
	for k in attr:
		...
		for p in array_data[k]:
			val+=p 

		average = val/len(array_data[k])
		dict_average[k] = average 
  	...
  	try:
		...
		if(not check_for_speech_in_string(sentence_from_speech)):
			...
			if sentence_from_speech in question_bank.keys():
				...
				#cyborgs_reply["reply"] = wordbank[sentence_from_speech]
				cyborgs_reply = {"reply":question_bank[sentence_from_speech],"mode":"smile"}
			...
			if not cyborgs_reply["mode"] in time_dict:
				time_dict[cyborgs_reply["mode"]] = []

			time_dict[cyborgs_reply["mode"]].append(time_end-time_start)

			print "Time: "+str(time_end-time_start)
			
			print "Array_size: "+str(len(time_array))
			if(len(time_array)==100):
				print "\n\n\n CORRECT PLOTTING CRITERIA\n\n\n"
				print time_dict
				print "len of data: "
				#dict_to_plot = return_average_computation_time(time_dict)
				plot_data(time_array)
				plot_average_computation_time(return_average_computation_time(time_dict))
...
def chuck_norris_database(): 
	...
	#For the Chuck Norris name within the jokes instead of NTNU Cyborg :

	#JSON_response = json.loads(requests.get("http://api.icndb.com/jokes/random/100").content)
...
#All types of variables: http://wiki.ros.org/msg

#Importere meldinger definert selv: from MY_PACKAGE_NAME.msg import MY_MESSAGE_NAME
# see this: https://github.com/ros/ros_comm/issues/320
